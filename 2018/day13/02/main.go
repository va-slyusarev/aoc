...
func tick(m *mine) (*point, bool) {
	...
	busy := make(map[point]*cart)
	for _, w := range m.workers {
		busy[w.p] = w
	}
	moves := make(map[point]*cart)
	toRemove := make(map[*cart]struct{})
	...
	newWorkers := make(carts, 0)
	for _, w := range m.workers {
		if _, exist := toRemove[w]; !exist {
