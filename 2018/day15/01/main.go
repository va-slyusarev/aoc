...
var speed = flag.Int("ms", 3000, "Speed visualization (ms).")
...
func (u *unit) attack(value *unit) { value.hp -= damage }
...
func newGame(lines []string) game {
	...
	for y, line := range lines {
		for x, r := range line {
			...
			switch r {
			case elf, goblin:
				g.players = append(g.players, &unit{p: p, kind: r, hp: hp})
				g.board[p] = cavern
			case wall, cavern:
				g.board[p] = r
			default:
