...
var generation = flag.Int("g", 50000000000, "Number of generations.")
...
func main() {
	...
	for g := 1; g <= *generation; g++ {
		...
		// take a close look at the result and identify the pattern of growth
		if g == step {
			fmt.Printf("Intermediate value is: %d, iteration: %d, time: %s\n", p.sumOrderSharp(), g, time.Since(t).Round(time.Millisecond))
			step *= 10
		}
	}

	fmt.Printf("Success! Target number is: %d, time: %s\n", p.sumOrderSharp(), time.Since(t).Round(time.Millisecond))
}
